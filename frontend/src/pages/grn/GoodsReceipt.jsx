...
export default function GoodsReceipt() {
  ...
  const fetchGRNList = async (page = 1) => {
    ...
    try {
      ...
      console.log('GRN Array:', grnArray, 'Count:', grnArray.length);
      setGrnList(grnArray);
      setTotalRecords(res.data.total || grnArray.length);
      setTotalPages(res.data.total_pages || Math.ceil(grnArray.length / itemsPerPage));
      setCurrentPage(page);
  ...
  const handlePOSelect = async (poNumber) => {
    ...
    if (selectedPO) {
      ...
      showToast('Loading items from PO...', 'info');
      
      // Fetch and populate PO items
      await fetchPODetails(poNumber);
    } else {
      showToast('PO not found in the list', 'error');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh the GRN list immediately
      fetchGRNList();
    } catch (err) {
                    ...
                    <div key={idx} className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200">
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          {form.with_po ? (
                            <input
                              placeholder="Enter item name or select PO to auto-load items"
                              value={item.item_name}
                              onChange={(e) => updateItem(idx, 'item_name', e.target.value)}
