...
export default function Billing() {
  ...
  const fetchBillings = async () => {
    try {
      ...
      setFilteredBillings(allBillings);
    } catch (err) {
  ...
  const handleFilterChange = (type) => {
    ...
    if (type === 'ALL') {
      ...
      const filtered = billings.filter(billing => {
        const returnType = billing.return_header?.return_type;
        switch (type) {
          case 'INTERNAL':
            return returnType === 'INTERNAL';
          case 'EXTERNAL':
            return returnType === 'EXTERNAL';
          case 'TO_CUSTOMER':
            return returnType === 'TO_CUSTOMER';
          case 'FROM_CUSTOMER':
            return returnType === 'FROM_CUSTOMER';
          case 'TO_VENDOR':
            return returnType === 'TO_VENDOR';
          default:
            return true;
        }
      });
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 p-6">
      ...
      {/* HEADER */}
      <div className="mb-4">
        <div className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-500 p-4 text-white shadow-md">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm uppercase opacity-80">Financial Management</div>
              <h1 className="text-2xl font-semibold mt-1">Billing System</h1>
              <p className="mt-1 opacity-90">Manage invoices, payments and billing records.</p>
            </div>
            <div className="flex items-center gap-3">
              <Link
                to="/app/billing/create"
                className="bg-white text-blue-600 px-3 py-2 rounded-lg font-medium hover:bg-blue-50 transition-colors flex items-center gap-2 text-sm"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-2 rounded-full text-sm">
                <span className="font-medium">Total</span>
                <div className="bg-white/20 px-2 py-1 rounded-full text-xs">{billings.length}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="min-w-full">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Return Type</th>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredBillings.map((billing) => (
                <tr key={billing.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {billing.return_header?.return_type && (
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        billing.return_header.return_type === 'INTERNAL' ? 'bg-blue-100 text-blue-800' :
                        billing.return_header.return_type === 'EXTERNAL' ? 'bg-green-100 text-green-800' :
                        billing.return_header.return_type === 'TO_CUSTOMER' ? 'bg-purple-100 text-purple-800' :
                        billing.return_header.return_type === 'FROM_CUSTOMER' ? 'bg-orange-100 text-orange-800' :
                        billing.return_header.return_type === 'TO_VENDOR' ? 'bg-red-100 text-red-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {billing.return_header.return_type.replace('_', ' ')}
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
        ...
        {filteredBillings.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-slate-900">
              {filterType === 'ALL' ? 'No invoices found' : `No ${filterType.replace('_', ' ').toLowerCase()} invoices found`}
            </h3>
            <p className="mt-1 text-sm text-slate-500">
              {filterType === 'ALL' 
                ? 'Get started by creating your first invoice.' 
                : 'Try selecting a different filter or create a new invoice.'}
            </p>
          </div>
        )}
      </div>
